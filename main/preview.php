...
POS
 ...
 <link href="css/bootstrap.css" rel="stylesheet">
  ...
  <link rel="stylesheet" href="css/font-awesome.min.css">
	...
	.sidebar-nav {
	background-color: rgba(72, 71, 71, 0.73);
	font:bold 15px 'Aleo';
    padding: 9px 5px;
	color: black;
      }
...
{ 
  var disp_setting="toolbar=yes,location=no,directories=yes,menubar=yes,"; 
      disp_setting+="scrollbars=yes,width=180, height=400, left=100, top=25"; 
  var content_vlue = document.getElementById("content").innerHTML; 
  
  var docprint=window.open("","",disp_setting); 
   docprint.document.open(); 
   docprint.document.write('</head><body onLoad="self.print()" style="width: 180px; font-size: 13px; font-family: arial;">');          
...
<div style="margin: 0 auto; padding: 20px; width: 900px; font-weight: normal;">
	...
	Fry Town <br>
	Dhanmondi, Dhaka , Bangladesh	<br>	<br>
	</center>
	<div>
	<?php
	...
	<table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		...
		<tbody>
				...
				<tr>
					...
					<?php
					$sdsd=$_GET['invoice'];
					$resultas = $db->prepare("SELECT grandtotal FROM sales WHERE invoice_number= :a");
					$resultas->bindParam(':a', $sdsd);
					$resultas->execute();
					for($i=0; $rowas = $resultas->fetch(); $i++){
					$fgfg=$rowas['grandtotal'];
				...
				<!--for vate-->
								<tr>
					<td colspan="4" style=" text-align:right;"><strong style="font-size: 12px;">Vat: &nbsp;</strong></td>
					<td colspan="2"><strong style="font-size: 12px;">
				<?php
				$sdsd=$_GET['invoice'];
					$resultas = $db->prepare("SELECT * FROM sales WHERE invoice_number= :a");
					$resultas->bindParam(':a', $sdsd);
					$resultas->execute();
					for($i=0; $rowas = $resultas->fetch(); $i++){
					$vat=$rowas['vat'];
				...
				<!--for discount-->
								<tr>
					<td colspan="4" style=" text-align:right;"><strong style="font-size: 12px;">Discount: &nbsp;</strong></td>
					<td colspan="2"><strong style="font-size: 12px;">
				<?php
				$sdsd=$_GET['invoice'];
					$resultas = $db->prepare("SELECT * FROM sales WHERE invoice_number= :a");
				...
				<tr>
					...
					$resultas->execute();
					for($i=0; $rowas = $resultas->fetch(); $i++){
					$change=$rowas['due_date']-$rowas['grandtotal'];
